...
pub mod requirement {
	...
	impl RequirementState {
		pub fn pass(requirement: crate::requirement::Requirement) -> RequirementState {
			RequirementState {
		...
		pub fn fail(requirement: crate::requirement::Requirement) -> RequirementState {
			RequirementState {
				requirement: requirement.clone(),
				matched_courses: vec![],
				status: Status::Fail,
	...
	pub trait Audit {
		fn check(&self, state: &AuditState) -> RequirementState;
	}
}
