...
def pretty_ms(ms, *, unit_count=None, compact=False, verbose=False, sec_digits=1, ms_digits=0, separate_ms=False, format_sub_ms=False, keep_decimals_on_whole_seconds=True):
  ...
  if separate_ms or format_sub_ms or ms < 1000:
    ...
    if format_sub_ms:
      ...
      ms_str = format(ms_and_below, f".{ms_digits}f") if ms_digits else math.ceil(ms_and_below)
      add(decimal.Decimal(ms_str), 'millisecond', 'ms', ms_str)
  else:
    sec = ms / 1000 % 60
    sec_fixed = format(sec, f".{sec_digits}f")
    sec_str = sec_fixed if keep_decimals_on_whole_seconds else re.sub(r"\.0+$", '', sec_fixed)
